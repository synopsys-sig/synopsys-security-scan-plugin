...
| `network_airgap`                    | When **network_airgap** is set **true**, <br/> 1. If Bridge exists in the default `<synopsys_bridge_install_directory>`, it is used. Otherwise, errors out. <br/> 2. If Bridge exists in the specified `<synopsys_bridge_install_directory>`, it is used. Otherwise, errors out. <br/> 3. In case of airgap, synopsys-bridge url can also be provided and considered (which could be internal artifactory url). And if version info is available in url or url directory, bridge caching is considered otherwise it downloads. |           
#### Note:
